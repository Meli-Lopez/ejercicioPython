...
def agregarNovedad(idEquipo, fecha, descripcion):
    if idEquipo in equipos:
        ...
        print("La novedad ha sido agregada con éxito al equipo con ID {}.".format(idEquipo))
    else:
...
def modificarEquipo(idEquipo, cargador, mouse, ambiente):
    if idEquipo in equipos:
        ...
        print("El equipo con ID {} no existe.".format(idEquipo)) #El .format se utiliza para insertar valores en posiciones especificas
...
def menu():
    while True:
        ...
        if opcion.lower() == 'a':
            ...
            # Se solicita la informacion para modificar un equipo y se llama a la funcion correspondiente.
    ...
    print("El programa ha terminado con éxito :D.")
